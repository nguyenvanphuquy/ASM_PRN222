        ...
        foreach (var meta in EmbeddingModels)
        {
            ...
            try
            {
                ...
                foreach (var c in working)
                {
                    try
                    {
                        var passage = PrepareText(meta.Id, c.Content, isQuery: false);
                        var vec = await provider.GetEmbeddingAsync(passage, meta.Id);
                        c.VectorJson = JsonSerializer.Serialize(vec);
                        embedded++;
                    }
                    catch
                    {
                        // bỏ chunk lỗi, tiếp tục
                    }
                }
                embedSw.Stop();
                ...
                if (embedded == 0)
                    throw new InvalidOperationException("Không embed được chunk nào. Kiểm tra API key/token trong Settings.");
